...
export default class ChatRoom {
  ...
  setup_events() {
    this.$chat_room.on('click', () => {
      console.log('Chat room clicked:', this.profile.room);
      
      if (typeof this.chat_space !== 'undefined') {
        console.log('Chat space already exists, rendering');
        this.chat_space.render();
      } else {
        console.log('Creating new chat space');
      ...
      // Only mark as read when user clicks on the chat room
      if (this.profile.is_read === 0) {
        console.log('Marking message as read from chat_room.js click handler');
