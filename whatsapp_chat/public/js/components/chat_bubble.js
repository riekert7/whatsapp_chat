export default class ChatBubble {
  ...
  change_bubble() {
    ...
    if (this.parent.is_open === false) {
      // If we're in chat space, first return to chat list
      if ($('.chat-space').is(':visible')) {
        console.log('Returning to chat list before hiding');
        if (this.parent.chat_list) {
          this.parent.chat_list.render_messages();
          this.parent.chat_list.render();
        }
      }
      
      // Then update bubble and hide widget
      this.$chat_bubble
        .attr({
          title: this.open_title,
        })
